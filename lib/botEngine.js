...
export async function sendTelegramMessage(token, chatId, text) {
  const res = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
    ...
    // parse_mode 'HTML' supaya rule/welcome/fallback message yang mengandung
    // tag HTML (<b>, <i>, <u>, <s>, <code>, <pre>, <a href="">, <blockquote>, dst)
    // dirender Telegram, bukan ditampilkan sebagai teks mentah.
    body: JSON.stringify({ chat_id: chatId, text, parse_mode: 'HTML' }),
  });
  const data = await res.json();

  // Kalau teksnya mengandung tag yang bikin HTML tidak valid (mis. "<3" atau
  // tag ketutup salah), Telegram menolak dengan error "can't parse entities".
  // Daripada bot diam total, kirim ulang sebagai plain text supaya pesan tetap sampai.
