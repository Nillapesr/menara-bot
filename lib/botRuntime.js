// ================================================================
//  BOTRUNTIME.JS - SUPPORT RICH MESSAGE + BUTTON WARNA + EDIT
//  Developer: @makloayam
// ================================================================
...
function buildContext({ token, update, chatId, env }) {
  ...
  const callbackMessageChatId = update.callback_query?.message?.chat?.id || null;
  ...
  return {
    ...
    async sendRichMessage(html, options = {}) {
      ...
      const result = await telegramCall(token, 'sendRichMessage', payload);
      // Simpan message_id hasil kirim biar bisa di-edit/hapus nanti
      if (result?.result?.message_id) {
        this._lastRichMessageId = result.result.message_id;
        this._lastRichChatId = chatId;
      }
      return result;
    },

    // ================================================================
    // EDIT RICH MESSAGE (HAPUS + KIRIM BARU OTOMATIS)
    // ================================================================
    async editRichMessage(html, options = {}) {
      // Ambil messageId dari parameter, atau dari callback, atau dari hasil kirim terakhir
      let targetMessageId = options.messageId || callbackMessageId || this._lastRichMessageId || update.message?.message_id;
      let targetChatId = options.chatId || callbackMessageChatId || this._lastRichChatId || chatId;

      // KALO GA ADA MESSAGE ID, LANGSUNG KIRIM BARU
      if (!targetMessageId) {
        return this.sendRichMessage(html, options);
      }

      try {
        // COBA EDIT RICH MESSAGE
        const payload = {
        ...
        return await telegramCall(token, 'editRichMessage', payload);
      } catch (error) {
        // KALO EDIT GAGAL (misal rich message ga support edit), HAPUS + KIRIM BARU
        console.log('⚠️ Edit rich message gagal, hapus + kirim baru:', error.message);
        ...
        try {
          ...
          console.log('⚠️ Gagal hapus pesan:', e.message);
        }
    ...
    // ================================================================
    // HAPUS PESAN (TERMASUK RICH MESSAGE)
    // ================================================================
    async deleteMessage(messageId) {
      const targetId = messageId || callbackMessageId || this._lastRichMessageId || update.message?.message_id;
      const targetChat = callbackMessageChatId || this._lastRichChatId || chatId;
