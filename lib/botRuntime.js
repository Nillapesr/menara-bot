...
// Referensi: https://core.telegram.org/bots/api#inlinekeyboardbutton
// Sejak Bot API 9.4, "style" (warna tombol) dan "icon_custom_emoji_id"
// juga jadi field resmi — jadi keduanya diteruskan apa adanya, bukan di-strip.
const TELEGRAM_BUTTON_FIELDS = [
  ...
  'callback_game',
  'pay',
  'style', // 'danger' | 'primary' | 'success' — Bot API 9.4+
...
function buildInlineKeyboard(buttons) {
  // buttons: [[{ text, url?, callback_data?, style? }]] atau array datar
  ...
  const cleanRows = rows.map((row) =>
    (row || [])
      ...
      .map((btn) => {
        ...
        // Buang style yang tidak valid (bukan salah satu dari 3 warna resmi)
        // supaya Telegram tidak menolak seluruh payload karena satu tombol.
        ...
        // Tombol wajib punya salah satu action; kalau tidak ada action
        // valid sama sekali, jadikan callback_data no-op supaya tidak
        // dianggap invalid oleh Telegram.
