...
/**
 * Mencari command yang cocok dengan pesan/callback masuk, berdasarkan trigger.
 * commands: [{ trigger, code }, ...]
 * Trigger diawali '/': harus persis sama (atau diikuti spasi) dengan teks pesan.
 * Trigger tanpa '/': cocok jika teks pesan mengandung trigger tsb (keyword bebas),
 * atau jika callback_data persis sama dengan trigger.
 */
export function findMatchingCommand(commands, { text, callbackData }) {
  const list = commands || [];
