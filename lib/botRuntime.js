...
const FORBIDDEN_PATTERNS = [
  ...
  /\bfs\s*\./,
  /\beval\s*\(/,
  // Hapus comment baris dibawah biar user bisa pake setInterval
  // /\bFunction\s*\(/,
];
...
function buildContext({ token, update, chatId, env }) {
  ...
  return {
    ...
    // ===== FITUR ANIMASI BUTTON WARNA =====
    // Fungsi buat bikin button ganti warna otomatis
    async startButtonAnimation(buttons, intervalMs = 2000) {
      ...
      // Hentikan animasi sebelumnya kalo ada
      if (this.storage?._animationInterval) {
        clearInterval(this.storage._animationInterval);
        delete this.storage._animationInterval;
      }
      ...
      this.storage._animationInterval = interval;
    ...
    // Fungsi buat stop animasi
    async stopButtonAnimation() {
      if (this.storage?._animationInterval) {
        clearInterval(this.storage._animationInterval);
        delete this.storage._animationInterval;
      }
